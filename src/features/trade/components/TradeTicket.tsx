...
export function TradeTicket({ market, initialOutcome }: TradeTicketProps) {
    ...
    const imageSrc = resolveMarketImageSrc(
        market.imageUrl,
        market.category,
        imageFailed,
    );
        ...
        <section className="space-y-5 rounded-2xl border border-line bg-panel p-5 shadow-threshold">
            <div className="space-y-3">
                ...
                <p className="text-xs text-muted">
                    Above the ceiling, only math survives.
                </p>
            </div>
                ...
                <div className="space-y-2 rounded-xl border border-line bg-panel2/70 p-3">
                    ...
                    {!worldId.isVerified ? (
                        <p className="text-xs text-muted">
                            ...
                            <Link
                                to="/portfolio"
                                className="underline decoration-line-strong underline-offset-4 text-text"
                            >
                                Portfolio
                            </Link>{" "}
            ...
            <PriceInput
                ...
                onChange={(limitPrice) =>
                    setTicket((prev) => ({ ...prev, limitPrice }))
                }
            ...
            <div className="grid grid-cols-2 gap-3 text-xs">
                <div className="rounded-xl border border-line p-3">
                    <p className="uppercase tracking-[0.14em] text-muted">
                        Market Payment
                    </p>
                    <p className="kpi-number mt-1">
                        {formatUsd(marketPayment)}
                    </p>
                ...
                <div className="rounded-xl border border-line p-3">
                    ...
                    <p className="kpi-number mt-1">
                        {formatNumber(priceImpactEstimate, 2)}%
                    </p>
                </div>
                <div className="rounded-xl border border-line p-3">
                    <p className="uppercase tracking-[0.14em] text-muted">
                    ...
                    <p
                        className={`kpi-number mt-1 ${projectedPnl >= 0 ? "text-positive" : "text-risk"}`}
                    >
                        {formatUsd(projectedPnl)}
                    </p>
                </div>
            </div>
            ...
            {!validation.isValid ? (
                <p className="text-xs text-risk">{validationText}</p>
            ) : null}
            ...
            <div aria-live="polite" className="min-h-4">
                {mutation.data ? (
                    <p className="text-xs text-positive">
                        Submitted: {mutation.data.txHash}
                    </p>
                ) : null}
                {mutation.isError ? (
                    <p className="text-xs text-risk">
                        Submission failed. Retry after state sync.
                    </p>
                ) : null}
            </div>
        </section>
